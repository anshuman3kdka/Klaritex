...
async function POST(request) {
  ...
  if (!allowed) {
    return secureJsonResponse({ error: "Too many requests. Please wait before trying again." }, { status: 429 });
  }
  const parsedBody = await parseJsonBodyWithLimit(request, 51200);
  if (!parsedBody.ok) {
  ...
  try {
    ...
    if (isProviderUnavailableError(message)) {
      if (message.toLowerCase().includes("missing gemini api key")) {
        return secureJsonResponse({ error: "Gemini API key is not configured on the server." }, { status: 503 });
      }
