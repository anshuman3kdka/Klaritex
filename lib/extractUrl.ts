...
function secureDnsLookup(
  ...
  dns.lookup(hostname, { ...lookupOptions, all: true }, (err, addresses) => {
    if (err) {
      return callback(new UrlExtractionError("Could not fetch content from URL.") as any, null as any);
    }
    ...
    if (lookupOptions.all) {
      ...
      return callback(null, addresses[0].address, addresses[0].family || 4);
    }
  });
}

const httpAgent = new http.Agent({ lookup: secureDnsLookup as any });
const httpsAgent = new https.Agent({ lookup: secureDnsLookup as any });
