  ...
  return (
    <div>
      ...
      <input
        ...
        aria-describedby={errorMessage ? "url-helper-text url-error-text" : "url-helper-text"}
      ...
      <p id="url-helper-text" className="font-ui mt-2 text-xs text-[var(--text-secondary)]">
        Klaritex analyzes the article&apos;s text only. It does not search the web or fact-check claims.
      </p>
      {errorMessage ? <p id="url-error-text" role="alert" className="font-ui mt-2 text-sm text-[var(--missing-color)]">{errorMessage}</p> : null}
    </div>
  );
}
