  ...
  return (
    <div>
      ...
      <input
        ...
        aria-describedby={errorMessage ? "url-input-helper url-input-error" : "url-input-helper"}
      ...
      <p id="url-input-helper" className="font-ui mt-2 text-xs text-[var(--text-secondary)]">
        Klaritex analyzes the article&apos;s text only. It does not search the web or fact-check claims.
      </p>
      {errorMessage ? <p id="url-input-error" role="alert" className="font-ui mt-2 text-sm text-[var(--missing-color)]">{errorMessage}</p> : null}
    </div>
  );
}
