  ...
  return (
    <div>
      ...
      <input
        ...
        aria-describedby={errorMessage ? "klaritex-url-hint klaritex-url-error" : "klaritex-url-hint"}
      ...
      <p id="klaritex-url-hint" className="font-ui mt-2 text-xs text-[var(--text-secondary)]">
        Klaritex analyzes the article&apos;s text only. It does not search the web or fact-check claims.
      </p>
      {errorMessage ? <p id="klaritex-url-error" role="alert" className="font-ui mt-2 text-sm text-[var(--missing-color)]">{errorMessage}</p> : null}
    </div>
  );
}
