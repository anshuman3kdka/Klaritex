...
## 2025-05-12 - IP Spoofing via Leftmost X-Forwarded-For Values
**Vulnerability:** The application was extracting the leftmost IP address from the `X-Forwarded-For` header to identify the client's IP. Because the leftmost value can be arbitrarily spoofed by a malicious client adding their own `X-Forwarded-For` header before the request reaches the trusted proxy, this allows attackers to bypass IP-based rate limiting or security controls.
**Learning:** In proxy chains, the trusted proxy appends the actual client IP to the *right* side of the `X-Forwarded-For` header. The rightmost value is the only one guaranteed to be set by the immediate upstream proxy and cannot be overridden by the client.
**Prevention:** Always extract the rightmost (last) value from the `X-Forwarded-For` header to determine the true client IP address, ensuring it reflects the value set by the trusted edge proxy.
