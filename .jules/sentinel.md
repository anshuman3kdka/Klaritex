...
## 2024-06-25 - SSRF TOCTOU (Time-of-Check Time-of-Use) via DNS Rebinding
**Vulnerability:** The application mitigated SSRF by resolving a domain to an IP address (`assertPublicDnsResolution`) and validating it, and then subsequently passing the domain to `fetch()`. This pattern is vulnerable to DNS rebinding (a Time-of-Check Time-of-Use vulnerability). An attacker's DNS server could return a safe IP during the check phase, and then return a malicious internal IP (e.g., `127.0.0.1`) with a very short TTL when `fetch()` performs its own DNS lookup.
**Learning:** Checking a hostname's IP resolution separately from the actual network connection is inherently unsafe. Node.js's network stack (and libraries like `node-fetch`) will perform their own DNS lookups unless explicitly instructed otherwise.
**Prevention:** To mitigate DNS rebinding in Node.js when fetching untrusted URLs, validate the IP address *exactly* at the time of connection. For `node-fetch`, this requires creating custom `http.Agent` and `https.Agent` instances with an overridden `lookup` function that performs the safety validation immediately before returning the resolved IP to the underlying socket.
