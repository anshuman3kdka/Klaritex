...
## 2026-05-01 - IP Spoofing via X-Forwarded-For Header
**Vulnerability:** The application was extracting the first (leftmost) IP from the `x-forwarded-for` header to identify the client's IP address. This allows malicious clients to spoof their IP by sending a fake `x-forwarded-for` header, bypassing IP-based rate limiting because the trusted proxy appends the real IP to the end of the list.
**Learning:** Leftmost IPs in the `x-forwarded-for` header can be spoofed by the client and cannot be trusted. The rightmost IP is safely appended by the closest trusted proxy.
**Prevention:** Always extract the last (rightmost) value from the `x-forwarded-for` header to prevent rate-limiting bypass and IP spoofing.
