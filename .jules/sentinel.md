...
## 2024-05-18 - Path Traversal in Post Slug Resolution
**Vulnerability:** The `getPostBySlug` function in `lib/posts.ts` resolved post filenames dynamically via `path.join(POSTS_DIR, `${slug}.md`)` without verifying that the resulting path remained within the intended `content/posts` directory. This permitted path traversal by passing slugs like `../../../package.json` to access arbitrary files on the system ending in `.md` (or other files if the `.md` was cleverly bypassed or the linear scan read them).
**Learning:** Resolving paths from user-provided inputs directly into file system reads inherently introduces path traversal risks. The assumption that slugs are safe since they are "just IDs" is flawed, especially when routed directly from URL parameters.
**Prevention:** Always explicitly check that the fully resolved absolute file path (using `path.resolve()`) begins with the expected absolute base directory path (including a trailing separator) before reading any file content when the path depends on user input.
