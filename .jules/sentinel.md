...
## 2025-02-27 - Rate Limit Bypass via X-Forwarded-For Spoofing
**Vulnerability:** IP spoofing in rate limit check by extracting the leftmost IP address from the X-Forwarded-For header, which allows malicious actors to easily bypass rate limits by supplying a spoofed header.
**Learning:** The leftmost value in the X-Forwarded-For chain is set by the client and cannot be trusted. The rightmost value is appended by the closest trusted proxy.
**Prevention:** Always extract the last (rightmost) value from the X-Forwarded-For header for security-sensitive logic like rate limiting and IP banning.
